...
# -----------------------------
# PowerPoint (.pptx) Generation Feature
# -----------------------------
def parse_slides_from_text(text: str):
    """
    Parses plain text or Markdown containing Slide outlines into structured slide objects.
    Matches lines starting with "Slide N:", "## Slide N:", "**Slide N:**", etc.
    """
    slides = []
    ...
    if not matches:
        ...
        for sec in sections:
            ...
            bullets = [
                ...
                for llist in lines[1:] 
                for l in [llist] 
                if l.strip().startswith(('-', '*', '•', '>')) or (len(l.strip()) > 0 and not l.strip().startswith('#'))
            ]
            slides.append({"title": title or "Presentation Slide", "bullets": bullets[:6]})
    else:
        for match in matches:
            ...
            for line in lines[1:]:
                ...
                if cleaned.startswith(('-', '*', '•', '1.', '2.', '3.', '4.', '5.', '6.', '7.', '8.', '9.', '>')):
                    bullet_text = re.sub(r'^(?:[-*•>]\s*|\d+\.\s*)', '', cleaned).strip()
                    if bullet_text:
                        bullets.append(bullet_text)
                elif len(bullets) < 8 and len(cleaned) < 150:
                    bullets.append(cleaned)
            slides.append({"title": title_line or "Slide Title", "bullets": bullets})
            
    return slides


def create_pptx_from_slides(slides, presentation_title="Otari Presentation"):
    prs = Presentation()
    ...
    p2 = tf.add_paragraph()
    p2.text = "Generated by Otari Cost-Aware Assistant"
    p2.font.size = Pt(20)
    ...
    for slide_data in slides:
        ...
        p_title.text = slide_data.get("title", "Slide Title")
        p_title.font.bold = True
        p_title.font.size = Pt(32)
        p_title.font.color.rgb = RGBColor(30, 41, 59)
        
        # Bullets
        body_box = slide.shapes.add_textbox(Inches(0.8), Inches(1.8), Inches(11.733), Inches(5.0))
        tf_body = body_box.text_frame
        tf_body.word_wrap = True
        
        bullets = slide_data.get("bullets", [])
        if not bullets:
            bullets = ["(No content points specified)"]
            
        for i, b_text in enumerate(bullets):
            p_bullet = tf_body.paragraphs[0] if i == 0 else tf_body.add_paragraph()
            p_bullet.text = b_text
            p_bullet.level = 0
            p_bullet.font.size = Pt(20)
            p_bullet.font.color.rgb = RGBColor(51, 65, 85)
...
def generate_ppt():
    """
    Receives JSON: { "text": "...", "title": "..." (optional) }
    Parses slide outlines and generates a real downloadable .pptx file.
    """
    data = request.json or {}
    text = (data.get("text") or "").strip()
    title = data.get("title", "Otari Presentation")
    
    if not text:
        return jsonify({"error": "No text content provided for PPT generation"}), 400
        
    slides = parse_slides_from_text(text)
    if not slides:
        slides = [{"title": title, "bullets": [text[:200]]}]
        
    ppt_buffer = create_pptx_from_slides(slides, presentation_title=title)
    
    safe_filename = re.sub(r'[^a-zA-Z0-9_\-]', '_', title).strip('_') or "Otari_Presentation"
    return send_file(
        ppt_buffer,
